...
const Programs: React.FC = () => {
  ...
  const handlePublish = async (program: Program) => {
    try {
      await publishEventMutation.mutateAsync(program._id);
      toast.success("Program published successfully");
    } catch (error) {
      console.log("Failed to publish program:", error);
      toast.error("Failed to publish program, Please try again.");
    }
  };

  const handleUnpublish = async (program: Program) => {
    try {
      await unpublishEventMutation.mutateAsync(program._id);
      toast.success("Program unpublished successfully");
    } catch (error) {
      console.log("Failed to unpublish program:", error);
      toast.error("Failed to unpublish program, Please try again.");
    }
  };
  ...
  const handleAdd = (
    data: Omit<Program, "_id" | "createdAt" | "updatedAt">
  ) => {
